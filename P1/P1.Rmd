...
```{r, warning=FALSE}
tableUSA <- table(DF_MIX$Region, DF_MIX$EdLevel)
tableUSA <- prop.table(tableUSA)
colnames(tableUSA) <- c("Associate degree", "Bachelor’s degree", "I never completed any formal education", "Master’s degree", "Other doctoral degree", "Primary/elementary school", "Professional degree", "Secondary school", "Some college/university study without earning a degree")
tableUSA

barplot(tableUSA, beside = TRUE,las=2, xlab='', ylab='Frecuencia relativa', col = c("lightblue", "mistyrose"))
legend('topright', legend=rownames(tableUSA), bty='n',
       fill=c("lightblue", "mistyrose"))
```

El gráfico de barras muestra que la mayoría de programadores tanto estadounidenses como latinoamericanos tienen licenciatura (Bachelor’s degree); sin embargo, los estadounidenses son una mayor cantidad. La mayor presencia estadounidense se repite en todos los otros grados salvo el "Professional degree" (título profesional), en el cual Latinoamerica tiene mayor representación que Estados Unidos.

### Tipo de empleo

```{r}
tableEmpleo <- table(DF_MIX$Region, DF_MIX$Employment)
tableEmpleo <- prop.table(tableEmpleo)
tableEmpleo
barplot(tableEmpleo, beside = TRUE, las=2, xlab='', ylab='Frecuencia relativa', col = c("lightblue", "mistyrose"))
legend('topright', legend=rownames(tableEmpleo), bty='n',
       fill=c("lightblue", "mistyrose"))
```

El gráfico de barras muestra que la mayoría de programadores tanto estadounidenses como latinoamericanos trabajan a tiempo completo; sin embargo, los estadounidenses son una mayor cantidad. La mayor presencia estadounidense se repite en todos los tipos de empleos.
