...
-   ¿Cual es la diferencia entre el genero y las edades de los desarrolladores profesionales según la región en la que viven?
...
-   ¿Cómo influye la región en la que viven los desarrolladores profesionales en su educación?
    ...
    -   ¿De que manera influye la región en la que viven los desarrolladores profesionales en la frecuencia con la que aprenden un nuevo lenguaje/framework?
...
-   ¿Cómo influye la región en la que habitan los desarrolladores profesionales en su trabajo?
    ...
    -   ¿Cómo influye la región en la que los desarrolladores profesionales viven en su situacion de busqueda laboral?
...
-   ¿Cómo son las diferencias entre relaciones de variables de las dos regiones?
    ...
    -   ¿Existe diferencia entre la relación que hay entre la edad de los desarrolladores prefesiones y los años que llevan programando profesionalmente?
    
    -   ¿Cual es la diferencia entre regiones de la relación que existe entre la edad y la remuneración anual de los programadores profesionales que trabajan tiempo parcial?
    
    -   ¿Cual es la diferencia que exise entre regiones de la relación entre los años de experiencia y el sueldo de los programadores que traban en empresas pequeñas?
...
Se puede ver que existe un gran porcentaje de observaciones incompletas (40.69%), sin embargo, a estas no les faltan mucho datos, pues solo tenemos 11.31% de datos faltantes. Por este motivo no se eliminarán las observaciones incompletas pues reduciría la representatividad de la muestra, así que lo que se hará será no considerar los datos NA para los indicadores en los que estos estén presentes.
...
Se Procederá a realizar una análisis de la variable 'Proceso de Incorporación' para Verificar las diferencias que hay en el acoplamiento de nuevos desarrolladores en las empresas u organizaciones de América Latina y Estados Unidos.
...
Podemos observar que la cantidad de programadores que buscan empleo es mayor en América Latina, y la cantidad de programadores que no buscan empleo es mucho mayor en Estados Unidos (17.71%). En cuanto a las otras opciones, también hay diferencia, pero es menor. Esta es de 5% respecto a las personas que buscan un empleo, siendo mayor para Latinoamérica. Igualmente para la última opción, existe mayor proporcion disponibilidad a aceptar oportunidades en Latinoamérica, con un 11.77% a comparación de USA.

En base al análisis, se llega a la hipótesis de que la región influye en la situación de búsqueda laboral. Resaltando, que en Latinoamérica, la proporción de programadores que no está buscando trabajo es mucho menor que la de Estados Unidos, con una diferencia de 17.97 puntos porcentuales 
...
Se realizara un análisis de la variable de "compensación anual" que es fundamental para llegar a una buena hipótesis respecto al trabajo y su relación con cada región. Asimismo solo se tomara en cuenta a los trabajadores full-time para una mejor comparación en este caso.

Esta variable esta en dolares para ambas regiones para obtener los salarios en dolares se usando el ratio de intercambio en el 2020-02-19, asumiendo 12 meses laborando y 50 semanas trabajando.

##### Variable: Compensación anual en dolares
...
hist(DF_TEMP_USA$ConvertedComp, breaks = 30, freq = FALSE, main = "Histograma del salario anual USA", ylab = "Frecuencia", xlab = "Salario anual en dolares")
abline(v=mean(as.numeric(DF_USA$ConvertedComp), na.rm = TRUE), col = "red")
hist(DF_TEMP_LA$ConvertedComp, breaks = 30, freq = FALSE, main = "Histograma del salario anual Latinoamerica", ylab = "Frecuencia", xlab = "Salario anual en dolares")
...
Se observa en los histogramas que existen valores muy atípicos respecto a la media (esta es afectado por estos datos). Por ello, para un mejor análisis, se decidió crear una nueva variable booleana isStandard que asigna TRUE a todos los valores por debajo de un Salario que se consideró máximo. Según la pagina ZipRecruiter el máximo Salario anual de un desarrollador full-time fue de 171,000 USD. Por otro lado, según la pagina Evalart, un desarrollador en América Latina en el país mejor pagado (Panamá) un desarrollador en su máximo nivel (Master) percibe 3500 USD mensuales, anualmente se usara 42000 usd. Estos valores también se ven reflejados en nuestros gráficos. Se consideró solo a los que trabajan tiempo completo.
...
```
En Estados Unidos se observa una distribución normal respecto a la media, es decir que la mayoría de programadores gana alrededor de 104533 dolares anuales. Por otro lado, en América Latina no existe una distribución muy atípica, pero sí hay más variación respecto a la media, a comparación de USA.
...
```
 ...
 Asimismo, se resalta la diferencia entre la media de USA y Latinoamérica, que en USA es el 580.77% de la de América Latina.
```{r}
...
#### Rreguntas que involucran relaciones entre variables
...
```
Sin embargo, si se desea comparar la relación entre las dos regiones, se tiene que usar la correlación, pues en este descriptor no influye la escala pues esta dividida por el promedio de los datos. Con la correlación podemos ver que es bastante similar y que, además en las dos regiones es bastante alta.
...
Para responder esta pregunta se procederá con el análisis de las variables de remuneración anual y edad considerando solo los que trabajan tiempo completo y además, tienen la variable de sueldo estándar utilizada para filtrar la variable del sueldo como TRUE.
...
Para responder a esta pregunta, se seleccionaron las variables de remuneración anual y años programando profesionalmente de las observaciones que tienen en la variable del tamaño de la organización como que trabajan en una empresa de entre 2 a 9 empleados o de 10 a 19 y una vez más se tomo solo a los programadores que contaban con un sueldo "estándar" segun la variable booleana que fue definida anteriormente.
...
```
Se decidió utilizar un diagrama de dispersión por colores por los motivos ya mencionados anteriormente. Se puede observar en el diagrama que la regresion lineal tiene pendiente positiva en ambos casos. Se recurre a las covarianza para ver el grado de asosciacion en las variables y se ve que en ambos casos es positiva, lo que indicaría que las variables crecen en la misma dirección. 
