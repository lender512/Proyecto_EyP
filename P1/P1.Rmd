...
#DF_USA_T[(as.numeric(DF_USA_T$Age) < as.numeric(DF_USA_T$Age1stCode) & !is.na(DF_USA_T$Age)),]
DF_LA_T <- DF_LA
plot(x = DF_USA_T$Age, y = DF_USA_T$Age1stCode)
abline(lm(Age ~ Age1stCode, data = DF_USA_T))
plot(x = DF_LA_T$Age, y = DF_LA_T$Age1stCode)
abline(lm(DF_LA_T$Age ~ DF_LA_T$Age1stCode))
DF_USA_T
cor(y = as.double(DF_USA_T$YearsCodePro),x = as.double(DF_USA_T$Age1stCode), use = "complete.obs")
cor(y = as.double(DF_LA_T$YearsCodePro),x = as.double(DF_LA_T$Age1stCode), use = "complete.obs")
```
```{r}
DF_USA_T <- DF_USA
#DF_USA_T[(as.numeric(DF_USA_T$Age) < as.numeric(DF_USA_T$Age1stCode) & !is.na(DF_USA_T$Age)),]
DF_LA_T <- DF_LA
plot(x = DF_USA_T$YearsCodePro, y = DF_USA_T$ConvertedComp, ylim = c (0,200000))
plot(x = DF_LA_T$YearsCodePro, y = DF_LA_T$ConvertedComp, ylim = c (0,200000))
abline(lm(DF_LA_T$YearsCodePro ~ DF_LA_T$ConvertedComp))
```
