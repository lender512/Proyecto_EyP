...
Luego del análisis de la variable, se puede presentar la hipótesis de que la mayoría de desarrolladores profesionales de Estados Unidos y la de los de América Latina tienen el grado académico de licenciados.
...
```{r}
plot(x = DF_USA_T$Age, y = DF_USA_T$YearsCodePro)
abline(lm(DF_USA_T$YearsCodePro ~ DF_USA_T$Age))
```
...
DF_LA_T <-DF_LA[DF_LA$ConvertedComp < 48000 & !is.na(DF_LA$ConvertedComp) & !is.na(DF_LA$Age),]
DF_USA <-DF_USA[DF_USA$ConvertedComp < 200000 & !is.na(DF_USA$ConvertedComp) & !is.na(DF_USA$Age),]
...
DF_USA_T$ConvertedComp[(DF_USA_T$ConvertedComp > 200000 | DF_USA_T$ConvertedComp == 0) & !is.na(DF_USA_T$ConvertedComp)] <- NA
DF_LA_T$ConvertedComp[(DF_LA_T$ConvertedComp > 200000| DF_LA_T$ConvertedComp == 0) & !is.na(DF_LA_T$ConvertedComp)] <- NA
