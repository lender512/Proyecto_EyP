...
Se decidió realizar un analistas de aspectos generales como lo son la edad y el género de los desarrolladores previo al del aspecto educativo y laboral para poder mostrar otras variables que también se consideraron importantes pero no se podían clasificar en esas dos categorías. 
...
Para finalizar con confirmacion de una diferencia real, se hace uso del parámetro *notch* del gráfico de cajas y bigotes para ver si la diferencia entre las medianas es estadísticamente significativas con un 95% de intervalo de confianza, si estas se sobreponen, significa que practicamente no hay diferencia de medianas, por el otro lado, si no se sobreponen si existe dicha diferencia. Este parámetro se seguirá utilizando a lo largo del estudio. En esta caso se aprecia que las muescas no se superponen, por lo que podemos calcular las medianas y determinar cual es la diferencia real, pues puede que los promedios estén sesgados por valores atípicos. Existe una diferencia de 3 años.
```{r, comment=NA}
...
En base al análisis de estas tres variables, se llega a la hipótesis de que en Estados Unidos es más accesible conseguir un empleo en empresas grandes, a diferencia de America Latina. Así mismo, trabajar como desarrollador en Estados Unidos puede resultar mas tedioso, ya que los desarrolladores de esta región se sienten mas insatisfechos con su empleo, a diferencia de America Latina, donde hay un menor grado de insatisfacción, y un mayor grado de neutralidad.
...
Para la limpieza de datos se utilizó el criterio de que una persona no puede tener más años codeando como profesional que años de vida, porque sería ilógico que alguien tenga más años codeando profesionalmente que años de vida.
...
Según la instancia federal del gobierno estadounidense (Internal Revenue Services) para que un trabajo sea considerado a tiempo completo mínimo tiene que cumplir 30 horas semanales, o 130 horas de servicio por mes y no hay un límite máximo de horas semanales. Por otra parte, en Latinoamérica según BBC new en ciertos países existen límites de 48 horas semanales  y en otros países en rango de horas es de 40 a 47 horas semanales. 
Por ende, se decidió limpiar algunos datos debido a que se encontró valores imposibles, como 168 horas a la semana. Por lo cual decidimos que el rango normal de horas semanales considerando 5 días laborales a la semana va de  30 (horas mínimas en estados unidos) a 50 (rango máximo  en latinoamerica añadiendo algunas horas extras) horas.
...
Para este apartado se utiliza la gráfica de caja porque ayuda a representar gráficamente variables cuantitativas mediante la distribución de sus cuartiles, además de que permite incluir los datos atípicos. Para la gráfica se consideró a los desarrolladores que trabajan a tiempo completo para que las unidades muestrales sean comparables. Según la gráfica, la media de horas que trabajan los encuestados por semana en América Latina es de 41.05 horas y en Estados Unidos es de 41.03 
```{r}
mean(DF_LA$WorkWeekHrs, na.rm = TRUE)
mean(DF_USA$WorkWeekHrs, na.rm = TRUE)
```
...
Para esta variable se hizo uso de un barplot por separado para cada región, debido a que existe una cantidad de lenguajes de programación considerable, Además un gráfico de barras nos permite visualizar en que lenguajes se trabajan más, para facilitar esta comparación se ordeno el barplot en orden descendente.
