...
hist(DF_FT_USA_STD$ConvertedComp[DF_FT_USA_STD$isStandard_usa == TRUE ], breaks = 15, probability = TRUE, main = "Histograma del salario anual USA", ylab = "Frecuencia", xlab = "Salario anual en dolares")
abline(v=mean(as.numeric(DF_FT_USA_STD$ConvertedComp), na.rm = TRUE), col = "red")
hist(DF_FT_LA_STD$ConvertedComp, breaks = 15, probability = TRUE, main = "Histograma del salario anual Latinoamerica", ylab = "Frecuencia", xlab = "Salario anual en dolares")
...
```
Se puede apreciar que la probabilidad de que al escoger un programador en Estados Unidos o en América Latina la empresa donde trabaja tenga un buen proceso de incorporación es bastante similar. Se puede extener este experimento para aplicar una distribución binomial, y mostrar las probabilidades de obtener de 1 a 10 programadores cuya empresa tenga un buen proceso de incorporación al escogerlos de un grupo de 10.
