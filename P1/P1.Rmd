...
DF_MIX$Age1stCode[!(DF_MIX$Age1stCode > DF_MIX$Age) & !is.na(DF_MIX$Age)] <- NA
DF_MIX$Age1stCode[DF_MIX$Age1stCode == 85] <- NA
DF_USA$Age1stCode[!(DF_USA$Age1stCode > DF_USA$Age) & !is.na(DF_USA$Age)] <- NA
DF_LA$Age1stCode[!(DF_LA$Age1stCode > DF_LA$Age) & !is.na(DF_LA$Age)] <- NA
...
Decidimos utilizar un boxplot ya que nos permite ver múltiples descriptores de manera eficiente. Nos fijamos en que la diferencia de las medianas para comparar los valores y vemos que son estadísticamente significativas y que se diferencian en 1 años. Además calculamos la media para ver el sesgo de la gráfica y vemos que al prácticamente coincidir con la media en el caso de América Latina, significa que la distribución es simétrica.
...
Para saber a que edad en promedio realizaron su primer programa se usará la media. Se puede notar que en América Latina la edad promedio es 15.9 y en Estados Unidos es 15.44, la diferencia es bastante pequeña.
...
```{r}
plot(x = DF_USA$Age, y = DF_USA$YearsCodePro, col = "red")
points(x = DF_LA$Age, y = DF_LA$YearsCodePro, col = "blue")
...
DF_LA_T <-DF_LA[DF_LA$ConvertedComp < 200000 & !is.na(DF_LA$ConvertedComp) & !is.na(DF_LA$Age),]
DF_USA_T <-DF_USA[DF_USA$ConvertedComp < 200000 & !is.na(DF_USA$ConvertedComp) & !is.na(DF_USA$Age),]
plot(x = DF_USA_T$Age, y = DF_USA_T$ConvertedComp, col = "red")
abline(lm(DF_USA_T$Age ~ DF_USA_T$ConvertedComp), col = "red")
points(x = DF_LA_T$Age, y = DF_LA_T$ConvertedComp)
DF_LA_T
...
#DF_USA_T$ConvertedComp[(DF_USA_T$ConvertedComp > 178000 | DF_USA_T$ConvertedComp == 0) & !is.na(DF_USA_T$ConvertedComp)] <- NA
#DF_LA_T$ConvertedComp[(DF_LA_T$ConvertedComp > 178000| DF_LA_T$ConvertedComp == 0) & !is.na(DF_LA_T$ConvertedComp)] <- NA
...
#abline(lm(ConvertedComp ~  YearsCodePro, data = DF_LA_T))
#abline(lm(ConvertedComp ~  YearsCodePro, data = DF_USA_T))
...
DF_MIX[DF_MIX$Region == "Estados Unidos",]
DF_FREE_USA <- DF_MIX[DF_MIX$isStandard_mix == TRUE & DF_MIX$Employment == "Independent contractor, freelancer, or self-employed" & !is.na(DF_MIX$Employment) & DF_MIX$Region == "Estados Unidos",]
plot(x = DF_FREE_USA$YearsCodePro, y = DF_FREE_USA$ConvertedComp, col = "red")
abline(lm(ConvertedComp ~  YearsCodePro, data = DF_FREE_USA))
cor(x = DF_FREE_USA$YearsCodePro,y = DF_FREE_USA$ConvertedComp, use = "pairwise.complete.obs")

DF_FREE_LA <- DF_MIX[DF_MIX$isStandard_mix == TRUE & DF_MIX$Employment == "Independent contractor, freelancer, or self-employed" & DF_MIX$Region == "América Latina",]
plot(x = DF_FREE_LA$YearsCodePro, y = DF_FREE_LA$ConvertedComp, col = "red")
abline(lm(ConvertedComp ~  YearsCodePro, data = DF_FREE_LA))
cor(x = DF_FREE_LA$YearsCodePro,y = DF_FREE_LA$ConvertedComp, use = "pairwise.complete.obs")
